...
class Rover:
    ...
    def run(self):
        ...
        self.lr_pwm = pwm.PWM(0)
        self.lr_pwm.set_period(50)

        self.rr_pwm = pwm.PWM(1)
        ...
        while True:
            ...
            if float(self.rcin.read(4)) > 1490:
                ...
                th_lr = min(1, max(0, thbound - yaw * 0.3 - drift * self.drift_gain))
                th_rr = min(1, max(0, thbound + yaw * 0.3 + drift * self.drift_gain))
